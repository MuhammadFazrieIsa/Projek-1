...
<head>
    ...
    <title>Approval Pembimbing</title>
...
if (isset($_POST['btnkirim'])) {
    ...
    $sidang = mysqli_real_escape_string($mysqli, $_POST['sidang']);
    ...
    if (!empty($nim)) {
        ...
        if (mysqli_num_rows($checkResult) > 0) {
            ...
            $query = "UPDATE status 
                      SET status_aprove_pembimbing = '$statusapprove', 
                          keterangan_pembimbing = '$keterangan', 
                          tgl_approve_pembimbing = '$tanggalapprove',
                          status_sidang_bikin = '$sidang'
                      WHERE nim = '$nim'";

            if (mysqli_query($mysqli, $query)) {
                echo "<script>toastr.success('Data updated successfully.');</script>";
                header("Location: tampilapprovalpembimbing.php"); 
            } else {
