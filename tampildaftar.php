...
<script>
    function submitStatus() {
        const nim = $('#modalTerima').hasClass('show') ? $('#modalTerima').data('nim') : $('#modalTidakTerima').data('nim');
        const status = $('#modalTerima').hasClass('show') ? 'terima' : 'tidak-terima';
        const comment = status === 'terima' ? $('#commentTerima').val() : $('#commentTidakTerima').val();

        // Make AJAX request to submit the status and comment
        $.ajax({
            url: 'simpandaftar.php', // PHP script to handle the status update
            method: 'POST',
            data: {
                nim: nim,
                status: status,
            ...
            success: function(response) {
                ...
                $('#modalTidakTerima').modal('hide');
                
                // Optionally, you can show a success notification
                toastr.success('Status Diupdate');
    ...
    $('#modalTerima').on('show.bs.modal', function(event) {
        ...
        const nim = button.data('nim');
        const status = button.data('status');
        $(this).data('nim', nim);
        $(this).data('status', status);
    });

    $('#modalTidakTerima').on('show.bs.modal', function(event) {
        const button = $(event.relatedTarget);
        const nim = button.data('nim');
        const status = button.data('status');
        $(this).data('nim', nim);
...
<div class="container-fluid text-center col-sm-12">
    <h1>Tampil Pendaftaran</h1>
    <p>Silahkan Mengecek Data Pendaftaran Mahasiswa di bawah ini</p>
    <div class="row">
        ...
        <table id="data-table" class="table table-striped table-bordered">
            ...
            <tbody>
                ...
                $query = "SELECT * 
                        FROM log AS pe
                        JOIN prodi AS pd 
                        ON pe.idprodi = pd.idprodi";
                $result = mysqli_query($mysqli, $query);
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_array($result)) {
                        echo "<tr>";
                        echo "<td>".$row['nomorpendaftaran']."</td>";
                        echo "<td>".$row['namamahasiswa']."</td>";
                        echo "<td>".$row['namaprodi']."</td>";
                        echo "<td>".$row['tanggaldaftar']."</td>";
                        echo "<td>".$row['nim']."</td>";
                        echo "<td>".$row['nomorhp']."</td>"; 
                        echo "<td>".$row['email']."</td>";
                        echo "<td>".$row['judulta']."</td>";
                        echo "<td class='form-container'>";

                        echo "<div id='buttons-".$row['nim']."'>";
                        if ($row['kondisi'] == 'terima') {
                            echo "<div id='status-".$row['nim']."' class='status-text'>Sudah Diterima</div>";
                        } elseif ($row['kondisi'] == 'tidak-terima') {
                            echo "<div id='status-".$row['nim']."' class='status-text'>Tidak Diterima</div>";
                        } else {
                            echo "<button type='button' class='btn btn-success' data-toggle='modal' data-target='#modalTerima' data-nim='".$row['nim']."' data-status='terima'>Terima</button>";|
                            echo "<button type='button' class='btn btn-danger' data-toggle='modal' data-target='#modalTidakTerima' data-nim='".$row['nim']."' data-status='tidak-terima'>Tidak Terima</button>";
                        }
                        echo "</div>";
                        echo "</td>";
                        echo "</tr>";
                    ...
                    echo "<tr><td colspan='11' class='text-center'>Tidak ada hasil</td></tr>";
                }
                ?>
            </tbody>
        </table>
...
<div class="modal fade" id="modalTerima" tabindex="-1" role="dialog" aria-labelledby="modalTerimaLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-dark" id="modalTerimaLabel">Terima Mahasiswa</h5>
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <label class="text-dark" for="commentTerima">Komentar:</label>
                <textarea id="commentTerima" class="form-control" rows="4"></textarea>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-success" id="submitTerima" onclick="submitStatus()">Terima</button>
            </div>
        </div>
    </div>
</div>

<!-- Modal for Tidak Terima -->
<div class="modal fade" id="modalTidakTerima" tabindex="-1" role="dialog" aria-labelledby="modalTidakTerimaLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-dark" id="modalTidakTerimaLabel">Tidak Terima Mahasiswa</h5>
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <label class="text-dark" for="commentTidakTerima">Komentar:</label>
                <textarea id="commentTidakTerima" class="form-control" rows="4"></textarea>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-danger" id="submitTidakTerima" onclick="submitStatus()">Tidak Terima</button>
            </div>
        </div>
    </div>
</div>
